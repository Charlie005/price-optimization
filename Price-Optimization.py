...
df  = pd.read_csv("https://raw.githubusercontent.com/Charlie005/price-optimization/master/coefs.csv")
...
# # For creating CSV file with coefs
# def load_data():   
#     global df
#     global df3
...
#     df3 = pd.DataFrame(columns=['NAME','Brand','MC','ZONE','Intercept','SP_Coef','UC_Coef','UC','UGST'])    
#     for k in dfdict:
...
#             mc = dfdict[k].MC.iloc[0]
#             brnd = dfdict[k].Brand.iloc[0]
#             minmrp = min(dfdict[k].MRP)
#             maxmrp = max(dfdict[k].MRP)
#             pricerange = str(minmrp) + ' - ' + str(maxmrp)
#             temp = {'NAME':k,'Brand':brnd,'MC':mc,'ZONE':i, 'Intercept':p[0], 'SP_Coef':p[1], 'UC_Coef':p[2], 'UC':cost, 'UGST':gst,'Price_Range':pricerange}
